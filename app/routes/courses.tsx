...
import { getCourseRatingStatsMultiple } from "~/services/ratingService";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const ratingStatsMap = getCourseRatingStatsMultiple(courses.map((c) => c.id));
  ...
  const coursesWithLessonCount = courses.map((course) => {
    ...
    const pppPrice = course.pppEnabled
      ...
      : course.price;
    const rating = ratingStatsMap.get(course.id);
    return {
      ...course,
